- 👋 Hi, I’m @joeyvigil
- 💻 Frontend: React
- 🗃️ Backend: Flask, FastAPI, LangChain, LangGraph
- 🎲 I like Making Board Games 
- 📫 How to reach me by email: joeygvigil@gmail.com

![codewars](https://www.codewars.com/users/joeyvigil/badges/large)

[![My Skills](https://skillicons.dev/icons?i=ai,angular,arduino,autocad,aws,bootstrap,css,docker,fastapi,flask,gcp,git,github,html,java,jenkins,js,materialui,nodejs,postman,postgres,py,r,react,spring,sqlite,tailwind,ts,vercel&perline=10)](https://skillicons.dev)

<!---
joeyvigil/joeyvigil is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
